...
class LakeAnalyzer:
    ...
    def create_linear_plot(self, original_df, df, lake_name, trend_result):
        ...
        # === 🔥 CRITICAL FIX: Ensure lowest point is visible ===
    ...
    def get_trend_predictions(self, non_outliers):
        ...
        if STL_AVAILABLE and len(non_outliers) >= MIN_FOR_STL:
            try:
                ...
                df_monthly = df_temp.resample('ME').mean().dropna()  # ✅ Fixed: 'ME' instead of 'M'
    ...
    def generate_summary_report(self):
        ...
        # Print to console
        print("\n" + "="*50)
        print("🏆 TOP 5 RISING LAKES (m/year)")
        print("="*50)
        for _, row in top_rising.iterrows():
            print(f"{row['Lake'][:30]:<30} {row['Annual_Trend_m_per_year']:>+8.3f} m/yr (R²={row['R_squared']:.3f})")

        print("\n" + "="*50)
        print("📉 TOP 5 FALLING LAKES (m/year)")
        print("="*50)
